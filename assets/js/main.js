jQuery(function($){
	...
	$('.closeReguAjax').click(function(e){
        e.preventDefault();
        $('html, body').stop().animate({scrollTop: $($(this).attr('href')).offset().top - NavOffset}, 500,"swing");
        $(this).hide();
        $('#reguAjax').slideUp();
	...
	$('.triggerRegu').click(function(e){
        ...
        $.ajax({
            ...
            dataType: 'html',
            success: function(data) {
                var targetPageContent = $('<div />').html(data).find('.pageContentArea #xv-regu'+key);
                $('#reguAjax').html(targetPageContent).slideDown();
                $('.trackLoading').hide();
                $('.closeNewsAjax').show();
                $('html, body').stop().animate({scrollTop: $("#reguAjaxWrapper").offset().top - NavOffset}, 1000,"swing");
